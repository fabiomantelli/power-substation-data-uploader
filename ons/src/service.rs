/// Windows Service integration for osc-server
...
pub mod windows_service {
    ...
    const SERVICE_NAME: &str = "OscServer";

    static GLOBAL_CONFIG: Mutex<Option<ServerConfig>> = Mutex::new(None);

    define_windows_service!(ffi_service_main, service_main);

    pub fn run_as_service(cfg: ServerConfig) -> anyhow::Result<()> {
        {
        ...
        runtime.block_on(async move {
            tokio::select! {
                result = run_server(cfg) => {
                    if let Err(e) = result {
                        tracing::error!("run_server encerrou com erro: {:#}", e);
                    }
                }
